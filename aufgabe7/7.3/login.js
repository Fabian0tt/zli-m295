...
app.get('/lends/:isbn', (req, res)=>{
    const isbn = req.params.isbn;
    res.json(lends.find(b => b.isbn === isbn));
});
...
app.post("/lends", (req, res) => {
    ...
    if(isValid) {
        ...
        // lends.push({id: id, customer_id:customer_id, isbn: isbn, borrowed_at: borrowed_at});
        res.send(201);
    }
});
...
app.post('/login',(req, res)=> {
    ...
    if (!((email === "desk@library.example") && (password === "m295"))) {
        return res.status(401).send("Falsche Email oder Password")
    }
